...
class _HomePageState extends State<HomePage> {
  ...
  // Cache for past journeys so they persist if Firestore briefly returns empty
  List<Journey> _pastJourneysCache = [];
  StreamSubscription<List<jm.JourneyModel>>? _pastJourneysSub;
  // Caches and subscriptions for active and created journeys
  ...
  // Cache to store original JourneyModel objects by their ID for applying
  final Map<String, jm.JourneyModel> _journeyModelsById = {};
  
  // Track my application status per journey: {journeyId: status}
  ...
  bool _emailVerified = true; // default true to avoid flash; updated in initState
  ...
  void initState() {
    ...
    // Subscribe to backend journeys and append them alongside the dummy data.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _subscribeToExploreJourneys();
      
      final provider = Provider.of<JourneyProvider>(context, listen: false);
      // Subscribe to past journeys and keep a cache so UI doesn't flash-empty
      ...
      // Subscribe to active journeys (open) and cache
      ...
      // Subscribe to all created journeys and cache
      ...
      // Subscribe to applied journeys to track status
      ...
      if (userId != null) {
        ...
        // Subscribe to inbox for unread count
      ...
      // Subscribe to my applications to track status updates
      if (userId != null) {
  ...
  String _priceRangeFilter = 'any'; // 'any', 'under15', '15to25', '25to40', 'over40'
  String _dateFilter = 'any'; // 'any', 'today', 'thisWeek', 'thisWeekend', 'nextWeek'
  String _sortOption = 'recent'; // 'recent', 'soonest', 'highestPay', 'nearest' (implied default if loc avail)
  ...
  // Sample journey data with diverse compensation types
  final List<Journey> _sampleJourneys = [
    // 1. Hourly Pay - Groceries
    Journey(
      id: 1,
  ...
  Future<void> _initLocation() async {
    try {
      ...
      if (mounted) {
        ...
        if (uid != null) {
          // Use try-catch to avoid crashing if rules deny update
          try {
  ...
  // Convert JourneyModel (from Firestore) to local Journey used in this page
  Journey _convertModelToJourney(jm.JourneyModel m) {
    // Store the original model in cache for later use (e.g., when applying)
    if (m.id != null) {
      _journeyModelsById[m.id!] = m;
    }
    
    // Use model properties directly where possible to avoid string parsing issues
    ...
    // Removed redundant TryParse since we have the objects from the model now
  ...
  List<Journey> _getFilteredAndSortedJourneys() {
    ...
    switch (_sortOption) {
      case 'soonest':
        // For demo, sort by date string (in real app, use DateTime)
      ...
      case 'recent':
      default:
        // Prioritize nearby journeys if location is available
        // Same logic as 'nearest' effectively, but maybe less strict or secondary sort?
        // For now, if location filter is active, we default to nearest in state update.
        // But if user explicitly picked 'recent', we respect that.
        // However, if user just wants "utilizing zip codes", we should check checking distance
                      ...
                      onPressed: () async {
                        ...
                        if (locText.isNotEmpty) {
                          if (kIsWeb) {
                            ...
                            // Fallback coords for some popular city or just skip
                            // For now, let's keep old coords or null
                          } else {
                            try {
  ...
  Widget _buildJourneyActionButton(BuildContext context, Journey journey) {
    ...
    // Case 1: Host viewing their own journey (Active)
    if (isHost && !isPast) {
      // Check if any applicants have been accepted
    ...
    // Case 2: Already Applied (Check status)
    if (isApplied) {
      final status = _myApplicationStatus[journey.firestoreId] ?? 'applied';
      
      if (status == 'accepted') {
        // Check if journey starts within 24 hours
        ...
        return SizedBox(
          ...
          child: ShadButton(
            onPressed: isChatAvailable ? () async {
              ...
              // Show message that chat is not yet available
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
    ...
    if (isPast) {
      ...
      // Check if current user is involved (Host or Accepted Companion)
      ...
      if (jm != null) {
        if (currentUser?.uid == journey.hostUserId) {
           ...
           if (jm.acceptedCompanionId != null) {
             ...
             revieweeName = 'Companion'; // Ideally fetch name
           }
    ...
    return SizedBox(
      ...
      child: ShadButton(
        onPressed: () {
          ...
          // Get the original JourneyModel from cache
  ...
  // Helper to show read-only review
  ...
  // Helper to open write-review dialog
  ...
  // My Journeys sub-tab index
  int _myJourneysSubTab = 0;

  Widget _buildMyJourneysTab() {
    return Column(
      children: [
        // Header with Create Journey button
        Container(
  ...
  // Merged Active Tab: Shows Ongoing (top) and Upcoming (bottom)
  Widget _buildMergedActiveJourneys() {
    final now = DateTime.now();
    final userId = FirebaseAuth.instance.currentUser?.uid;

    // Filter applied journeys: only include if the user is the accepted companion
    ...
    // Prioritize _createdJourneysCache (Host view) and acceptedAppliedJourneys (Applicant view) over generic active cache
    ...
    final ongoing = uniqueJourneys.where((j) {
      ...
      // Using !isBefore/!isAfter for inclusive comparison (start <= now && now <= end)
    ...
    // Case 1: Neither ongoing nor upcoming exist
  ...
  // Active = Ongoing journeys (start time <= now <= end time)
  // Created = Upcoming journeys (future start time)
  ...
  Widget _buildAppliedJourneys() {
    ...
    return StreamBuilder<List<jm.JourneyModel>>(
      // We need to combine two streams here ideally, or use current cache.
      // For simplicity/performance, we'll use the applied stream + local check on created cache
      stream: journeyProvider.getAppliedJourneysStream(userId),
      builder: (context, snapshot) {
        // Collect applied journeys
        List<Journey> appliedList = [];
        if (snapshot.hasData) {
          appliedList = snapshot.data!.map((m) => _convertModelToJourney(m)).toList();
        } else if (_appliedJourneyIds.isNotEmpty) {
           // Fallback to what we might have found in global list
           // (This is a simplified fallback)
        }

        // Collect hosted journeys with applicants
        // We use _createdJourneysCache which is populated by MyAllJourneys stream in initState
        // We need to check 'currentApplicants' > 0 (which needs to be in UI model, or use raw model)
        // Since we only cache 'Journey' UI objects in _createdJourneysCache, we need to inspect them.
        // But 'Journey' UI object doesn't have 'currentApplicants'.
        // Let's rely on _journeyModelsById for raw data if possible, or just add currentApplicants to Journey.
        // OPTION: Add currentApplicants to Journey class.
        
        // Collect active hosted journeys so creator can view/manage them
        final now = DateTime.now();
  ...
  Widget _buildPastJourneys() {
    ...
    return StreamBuilder<List<jm.JourneyModel>>(
      ...
      builder: (context, snapshot) {
        ...
        // Strict client-side filter for Past Journeys to ensure no upcoming journeys leak in
        hostedPast = hostedPast.where((j) => j.endTime != null && j.endTime!.isBefore(now)).toList();

        // 2. Get Accepted Applied Past Journeys
        // Filter from the global _appliedJourneysCache (which contains all applied journeys)
        ...
        // Sort by date/time descending (most recent past first)
        uniquePast.sort((a, b) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, idx) {
            // This card will automatically show "WRITE REVIEW" for participants 
            // and "SEE REVIEW" for others, based on _buildJourneyActionButton logic.
  ...
  Widget _buildApplicationStatusBadge(String? journeyId) {
    // Default to 'Applied' if state not found yet
  ...
  // Check if profile is complete before allowing journey actions
  Future<void> _checkProfileAndProceed({
    required String actionType, // 'create' or 'join'
  ...
  // Show prompt when profile is incomplete
          ...
          actions: [
            ...
            ShadButton(
              onPressed: () async {
                ...
                // If profile was completed, refresh state
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              // Optional: Add "New Message" button (though usually contextual)
            ],
          ),
        ),
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                // If it's a permission error, it usually means rules are blocking active read
                return Center(
                  child: Padding(
                    padding: const EdgeInsets.all(16.0),
                    child: Text(
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  // We might not have 'otherUserName' in chat doc yet if created before my update, 
                  // but we can try to fetch or default.
                  // For now, let's just use "User" if not in 'participantData' (which I also added as a placeholder map).
                  
                  // In a real app, we'd fetch the user profile here or store snapshot in chat doc.
                  // Since I didn't enforce name storage in `getOrCreateChat`, let's do a quick FutureBuilder wrapper 
                  // OR just show "User" for now to keep it snappy, 
                  // OR fetch it if we want to be fancy.
                  
                  // Let's rely on FutureBuilder for the name to be nice.
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              // Only show chat button if accepted
                              if (status == 'accepted') ...[
                                Builder(
                                  builder: (context) {
                                    // Check if journey starts within 24 hours
                   ...
                   ShadButton(
                     onPressed: () {
                       ...
                       PaymentService().makePayment(
                         context: context, 
                         amount: '0', // 0 for setup intent usually
                         currency: 'usd'
                       );
                     },
                     child: Row(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                // TODO: Implement SMS sending using url_launcher
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
...
class _SupportChatDialogState extends State<SupportChatDialog> {
  ...
  Future<void> _handleSendMessage() async {
    ...
    // Forward to admins (valcourtjohnpeterson, syb)
